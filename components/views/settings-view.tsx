...
import { pushDataToSupabase, pullDataFromSupabase, resolveSupabaseConflict } from '@/lib/supabase-store'
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>通过 Firebase 在设备间同步数据</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                {syncStore.isEnabled ? (
                  <Cloud className="h-4 w-4 text-chart-1" />
                ) : (
                  <CloudOff className="h-4 w-4 text-muted-foreground" />
                )}
                <div>
                  <p className="text-sm font-medium">启用云同步</p>
                  <p className="text-xs text-muted-foreground">自动将数据同步到云端</p>
                </div>
              </div>
              <Switch
                checked={syncStore.isEnabled}
                onCheckedChange={(checked) => syncStore.setSyncEnabled(checked)}
                disabled={!getIsFirebaseConfigured()}
              />
            </div>
              ...
              <div className="space-y-3">
                <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3">
                  ...
                  <p className="text-[11px] text-muted-foreground mt-1">
                    请填写下方 Firebase 配置信息后保存，即可启用云同步功能
                  </p>
                </div>
                ...
                >
                  ...
                  {showFirebaseConfig ? '收起配置' : '填写 Firebase 配置'}
                </Button>
              ...
              <>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {syncStore.status === 'idle' && '未同步'}
                      {syncStore.status === 'syncing' && '正在同步...'}
                      {syncStore.status === 'synced' && '已同步'}
                      {syncStore.status === 'error' && `同步出错：${syncStore.error || '未知错误'}`}
                      {syncStore.status === 'offline' && '离线模式'}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <Badge
                      variant={
                        syncStore.status === 'synced' ? 'default' :
                        syncStore.status === 'error' ? 'destructive' :
                        syncStore.status === 'syncing' ? 'secondary' :
                        'outline'
                      }
                      className="text-[10px]"
                    >
                    ...
                    onClick={async () => {
                      try {
                        ...
                        if (cloudData) {
                          ...
                          const dataKeys = ['tasks', 'habits', 'habitCheckIns', 'timeEntries', 'pomodoroSessions', 'projects', 'anniversaries', 'goals', 'tags', 'reminders'] as const
                          dataKeys.forEach(key => {
                            if ((store as any)[key]) localData[key] = (store as any)[key]
                          })
            ...
            {!syncStore.isEnabled && (
              <div className="rounded-xl bg-muted/30 p-3">
                <p className="text-xs text-muted-foreground">
                  💡 启用云同步后，数据将自动上传到 Firebase 云端，支持多设备同步
                </p>
              </div>
            )}
          </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>通过 Supabase 自托管数据库同步数据（与 Firebase 并存）</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
            ...
            >
              <Settings className="h-3.5 w-3.5" />
              {showSupabaseConfig ? '收起配置' : '配置 Supabase'}
            </Button>
              ...
              <>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground">
                      ...
                      {supabaseStatus === 'idle' && (supabaseUserId ? `用户：${supabaseUserId.slice(0, 8)}...` : '点击下方按钮启动同步')}
                    </span>
                  </div>
                </div>
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        ...
                        const keys = [
                          'tasks', 'habits', 'habitCheckIns', 'timeEntries',
                          'pomodoroSessions', 'projects', 'anniversaries', 'goals',
                          'tags', 'reminders', 'journals', 'focusGoals',
                          'dailyReviewSettings',
                        ] as const
                        keys.forEach((k) => { if ((store as any)[k] !== undefined) data[k] = (store as any)[k] })
                        const userId = await import('@/lib/supabase').then((m) => m.ensureSupabaseAuth())
                        if (!userId) throw new Error('无法获取用户')
                    ...
                    onClick={async () => {
                      try {
                        ...
                        if (cloudData) {
                          ...
                          dataKeys.forEach((k) => { if ((store as any)[k]) localData[k] = (store as any)[k] })
                ...
                <div className="rounded-xl bg-muted/30 p-3 space-y-1.5">
                  ...
                  <p className="text-[11px] text-muted-foreground leading-relaxed">
                    ...
                    • 推/拉数据基于本设备匿名 ID 关联，请在多设备间保持同一匿名 ID<br />
