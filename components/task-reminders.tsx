  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 text-sm text-secondary-foreground">
        ...
        {reminders.length > 0 && (
          <span className="text-xs text-muted-foreground">({reminders.length} 个)</span>
        )}
      </div>
      ...
      {reminders.length === 0 && !showAdd && (
        <button
          ...
          className="w-full text-left text-sm text-muted-foreground hover:text-foreground border border-dashed border-border rounded-lg px-3 py-2 flex items-center gap-2"
        >
          ...
          {!showAdd && (
            <li>
              <button
                type="button"
                onClick={() => setShowAdd(true)}
                className="w-full text-left text-xs text-muted-foreground hover:text-foreground border border-dashed border-border rounded-lg px-3 py-1.5 flex items-center gap-1.5"
              >
      ...
      {showAdd && (
        <div className="bg-muted/60 rounded-lg p-3 space-y-3">
          ...
          {mode === 'preset' ? (
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {PRESET_TIMES.map(p => (
                  <button
                    ...
                    disabled={!hasDueDate}
                    className="text-xs px-2 py-1.5 bg-background text-foreground rounded border border-input hover:border-primary disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {p.label}
                  </button>
                ))}
            ...
            <div className="space-y-2">
              ...
              <button
                ...
                className="w-full text-sm bg-primary hover:bg-primary/90 text-primary-foreground rounded px-3 py-1.5"
              >
                添加绝对时间提醒
              </button>
            </div>
